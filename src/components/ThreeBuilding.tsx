...
export default function ThreeBuilding({ rooms, onRoomClick, dronePath }: ThreeBuildingProps) {
  ...
  useEffect(() => {
    ...
    const floorLight = new THREE.PointLight(0x5EDEFF, 0.5, 60);
    ...
    const createFireIcon = () => {
      ...
      for (let i = 0; i < 4; i++) {
        const flame = new THREE.Mesh(new THREE.ConeGeometry(0.2, 0.6, 8), fireMat);
        flame.position.set((i - 1.5) * 0.2, 0.3, 0);
    ...
    const createWarningIcon = () => {
      const group = new THREE.Group();
      const triGeo = new THREE.ConeGeometry(0.4, 0.7, 3);
    ...
    const createCheckIcon = () => {
      ...
      const part1 = new THREE.Mesh(new THREE.BoxGeometry(0.15, 0.3, 0.15), mat);
      part1.rotation.z = -Math.PI / 4;
      part1.position.set(-0.15, 0.25, 0);
      group.add(part1);
      const part2 = new THREE.Mesh(new THREE.BoxGeometry(0.15, 0.6, 0.15), mat);
      part2.rotation.z = Math.PI / 4;
      part2.position.set(0.1, 0.35, 0);
      group.add(part2);
      return group;
    };
    ...
    const createDrone = () => {
      ...
      const dLight = new THREE.PointLight(0x5EDEFF, 2, 12);
